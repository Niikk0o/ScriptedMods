class SpawnCoolDownSystem extends AdminModuleManager
{	
	...
	{
		ref map<string, ref map<string,float>> m_MapSpawnPoints = SpawnPointsManager.Cast(m_MissionServer.GetModule(SpawnPointsManager)).GetMap();
		ref map<string,float> PosInfo;
	    for (int i = 0; i < m_MapSpawnPoints.Count(); ++i)
	    {
	    	string PosName = m_MapSpawnPoints.GetKey(i);
	    	if (PosName == LocationName)
	    	{
	    		PosInfo = m_MapSpawnPoints.Get(PosName);
    		    for (int x = 0; x < PosInfo.Count(); ++x)
    		    {
    		    	string PosXyZ = PosInfo.GetKey(x);
    		    	float fTime   = PosInfo.Get(PosXyZ);
    		    }
	    	}
	    }
		return fTime;
